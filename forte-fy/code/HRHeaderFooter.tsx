...
export const HRHeader: React.FC<HRHeaderProps> = ({ navigate, isDark, setIsDark }) => {
  ...
  // Filter out the current department (HR) to show links to others
  const otherDepartments = DEPARTMENT_LIST.filter(d => d.id !== 'hr');

  // HR Department Specific Styling (Purple/Violet Theme)
  const theme = {
    bg: isDark ? 'bg-black/80 border-b border-[#bf00ff]/20' : 'bg-[#fdfaff]/90 border-b border-purple-200',
    text: isDark ? 'text-zinc-400 hover:text-[#bf00ff]' : 'text-purple-900/60 hover:text-[#bf00ff]',
    separator: isDark ? 'bg-white/10' : 'bg-black/10',
    logoBorder: isDark ? 'border-[#bf00ff]/50' : 'border-[#bf00ff]/30',
    logoBg: '#bf00ff', // Purple
    logoText: isDark ? 'text-white group-hover:text-[#bf00ff]' : 'text-slate-900 group-hover:text-[#bf00ff]',
    navLink: isDark ? 'text-zinc-300 hover:text-white' : 'text-purple-900/70 hover:text-[#bf00ff]',
    toggleBtn: isDark ? 'bg-white/10 hover:bg-[#bf00ff] text-white' : 'bg-white hover:bg-[#bf00ff] text-purple-900 hover:text-white border border-purple-100',
    mobileMenuBg: isDark ? 'bg-[#050505]/98 text-white' : 'bg-[#fdfaff]/98 text-purple-900'
  };
      ...
      {isMenuOpen && (
        <div className={`fixed inset-0 z-[10001] ${theme.mobileMenuBg} backdrop-blur-3xl flex flex-col p-8 animate-fade-in`}>
           <div className="flex justify-between items-center mb-12">
              <span className="font-heading font-black text-xl uppercase italic tracking-tighter text-[#bf00ff]">Forte-FY</span>
              <button onClick={() => setIsMenuOpen(false)} className="p-2 text-[#bf00ff]"><X size={28} /></button>
           </div>
           <div className="flex flex-col gap-6 items-center justify-center flex-1">
              ...
              {otherDepartments.map(dept => (
                <button
                  ...
                  className="text-2xl font-heading font-black uppercase italic tracking-tighter hover:text-[#bf00ff] transition-colors"
                >
                  {dept.name}
                </button>
              ))}
