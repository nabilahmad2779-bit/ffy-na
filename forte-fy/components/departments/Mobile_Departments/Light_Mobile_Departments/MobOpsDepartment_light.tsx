    ...
    <div ref={containerRef} className="min-h-screen pb-24 bg-[#fff8f0] text-slate-900 transition-colors duration-500 font-sans overflow-x-hidden">
      ...
      <section className="relative pt-32 pb-20 px-6 overflow-hidden flex flex-col items-center justify-center text-center">
        ...
        <div className="relative z-10 w-full">
           <div className="ops-reveal flex items-center justify-center gap-2 mb-4">
              ...
              <span className="text-xs font-mono font-bold tracking-[0.4em] uppercase text-slate-400">Department of</span>
           </div>
           
           <h1 className="ops-reveal text-6xl font-heading font-black uppercase italic tracking-tighter leading-[0.85] mb-8 text-slate-900">
              Operations
           </h1>
           
           <div className="ops-reveal w-16 h-1 bg-orange-600 mb-8 rounded-full mx-auto" />
           
           <p className="ops-reveal text-lg font-light italic text-slate-600 leading-relaxed max-w-xs mx-auto">
      ...
      <section className="px-6 py-12 border-t border-orange-200 bg-white scroll-reveal-section">
         <div className="flex flex-col items-center text-center mb-8">
            ...
            <div className="relative w-48 h-48 mb-8 rounded-full border-2 border-orange-200 p-2 shadow-lg">
               <img 
                  src="https://i.postimg.cc/3xyqGVyh/cca44d09-d92f-4c0d-987a-b8520bb2f212.jpg?auto=format&fit=crop&q=80" 
                  alt="Director" 
                  className="w-full h-full object-cover rounded-full"
               />
               <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 bg-white px-4 py-1 border border-orange-200 rounded-full shadow-sm">
                  <span className="text-[9px] font-black text-orange-600 uppercase tracking-widest">Head of Ops</span>
               </div>
            </div>
            ...
            <div className="relative max-w-sm mx-auto mt-4">
               <Quote className="absolute -top-4 -left-2 text-orange-200" size={30} />
               <p className="text-sm font-light italic leading-relaxed text-slate-600 px-4">
      ...
      {/* DEPARTMENT MANDATE */}
      <section className="px-6 py-12 border-t border-orange-200 bg-[#fff8f0] scroll-reveal-section text-center">
         ...
         <div className="p-6 border-l-2 border-orange-400 bg-white shadow-sm rounded-r-xl text-left">
            <h3 className="text-sm font-bold uppercase mb-3 text-slate-900 text-center">Foundation Philosophy</h3>
            <p className="text-sm font-light leading-relaxed text-slate-600 mb-4 text-justify">
            ...
            <p className="text-sm font-light leading-relaxed text-slate-600 text-justify">
      ...
      <section className="py-20 scroll-reveal-section border-t border-orange-200 bg-white overflow-hidden">
        <div className="px-6 text-center mb-12">
           ...
           <p className="text-[10px] font-mono text-slate-500 uppercase tracking-widest mt-2 flex items-center justify-center gap-2">
              <Zap size={10} className="text-orange-600" /> Tactical Inventory
           </p>
        </div>
        ...
        >
           {CAPABILITIES.map((cap, i) => {
             ...
             if (position === 0) {
                ...
                styleClass = "opacity-60 scale-90 z-10 translate-x-[20px] translate-y-2 rotate-y-12 blur-[1px]";
             } else if (position === -1) {
                styleClass = "opacity-0 scale-90 z-10 -translate-x-[20px]";
             }
             ...
             return (
               ...
               >
                  <div className="flex flex-col p-8 rounded-2xl border border-orange-100 bg-white shadow-xl relative overflow-hidden h-full min-h-[400px]">
                      {/* Decorative BG */}
                      <div className="absolute top-0 right-0 p-8 opacity-10">
                         <cap.icon size={120} className="text-orange-600" />
                      </div>
                      ...
                      <div className="relative z-10 flex flex-col h-full">
                         <div className="flex items-center justify-between mb-6">
                            ...
                            <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Protocol 0{i+1}</span>
                         </div>
                         
                         <h4 className="font-heading font-black text-2xl uppercase text-slate-900 mb-4 leading-none">{cap.title}</h4>
                         <div className="h-0.5 w-12 bg-orange-200 mb-6" />
                         
                         <p className="text-sm font-light text-slate-600 leading-relaxed text-justify flex-1">
                            {cap.desc}
                         </p>

                         <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between">
                            <span className="text-[8px] font-mono text-orange-600 uppercase">Status: Active</span>
        ...
        <div className="flex justify-center gap-2 mt-4">
           {CAPABILITIES.map((_, i) => (
              <div 
                key={i} 
                className={`h-1 rounded-full transition-all duration-300 ${i === activeIndex ? 'w-8 bg-orange-600' : 'w-2 bg-slate-200'}`}
              />
           ))}
        </div>
      </section>
      ...
      <section className="px-6 py-12 border-t border-orange-200 bg-orange-50 scroll-reveal-section">
         <div className="text-center mb-10">
            <div className="inline-block p-4 rounded-full mb-6 bg-white border border-orange-200 text-orange-600 shadow-md">
               <Briefcase size={28} />
            </div>
         ...
         <div className="space-y-4">
            {ROLES.map((role) => (
               <div key={role.id} className="group border border-orange-100 rounded-2xl overflow-hidden bg-white shadow-sm hover:shadow-md transition-all">
                  <div 
                    onClick={() => setExpandedRole(expandedRole === role.id ? null : role.id)}
                    className="flex items-center justify-between px-6 py-6 cursor-pointer"
                  >
                     <div className="flex items-center gap-4">
                        <div className="p-2 rounded-lg bg-orange-50 text-orange-600"><Activity size={18} /></div>
                        <div>
                           <h3 className="text-lg font-bold text-slate-900">{role.title}</h3>
                           <div className={`flex items-center gap-2 mt-1 text-[9px] font-black uppercase tracking-wider ${role.available ? 'text-green-600' : 'text-slate-400'}`}>
                              {role.available ? <ScanLine size={10} className="animate-pulse" /> : <Lock size={10} />}
                              {role.status}
                           </div>
                        </div>
                     </div>
                     <ChevronDown size={18} className={`text-slate-400 transition-transform ${expandedRole === role.id ? 'rotate-180' : ''}`} />
                  </div>
                  ...
                  <div className={`overflow-hidden transition-all duration-300 ${expandedRole === role.id ? 'max-h-[1200px] opacity-100' : 'max-h-0 opacity-0'}`}>
                     <div className="px-6 pb-8 pt-2 border-t border-orange-50 bg-orange-50/30">
                        <p className="text-sm text-slate-600 font-serif leading-relaxed mb-8">{role.desc}</p>
                        ...
                        {role.available ? (
                           <button 
                              ...
                              className="w-full py-4 rounded-xl bg-orange-600 text-white font-black uppercase text-xs tracking-widest flex items-center justify-center gap-2 hover:bg-slate-900 transition-all active:scale-95 shadow-lg shadow-orange-500/20"
                           >
                              Apply Now <ArrowUpRight size={14} />
                           </button>
                        ) : (
                           <div className="w-full py-4 rounded-xl border border-slate-200 text-slate-400 font-black uppercase text-xs tracking-widest flex items-center justify-center gap-2 bg-white">
