...
export const AlumniPageLight: React.FC = () => {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-slate-50 text-slate-900 overflow-x-hidden font-sans selection:bg-[#0066cc] selection:text-white">
      ...
      <section className="relative h-[80vh] flex flex-col items-center justify-center px-6 overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(0,102,204,0.05)_0%,transparent_70%)]" />
        
        {/* Floating Orbs */}
        <div className="floating-orb absolute top-1/4 left-1/4 w-64 h-64 bg-blue-100 rounded-full blur-[100px] opacity-60" />
        <div className="floating-orb absolute bottom-1/4 right-1/4 w-96 h-96 bg-cyan-100 rounded-full blur-[120px] opacity-60" />
        
        <div className="relative z-10 text-center max-w-5xl">
          <div className="alumni-hero-text inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-200/50 border border-slate-300 mb-8">
            <GraduationCap size={16} className="text-[#0066cc]" />
            <span className="text-xs font-bold uppercase tracking-widest text-slate-500">Our Legacy</span>
          </div>
          <h1 className="alumni-hero-text text-6xl md:text-[8rem] font-black tracking-tighter leading-[0.8] mb-8 uppercase italic">
            THE <span className="text-transparent [-webkit-text-stroke:1px_#0f172a]">ALUMNI</span> <br />
            <span className="text-[#0066cc]">NETWORK</span>
          </h1>
          <p className="alumni-hero-text text-xl md:text-2xl font-light text-slate-500 max-w-2xl mx-auto leading-relaxed">
      ...
      <section className="alumni-grid-section py-32 px-6 md:px-12 relative">
        <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 md:gap-24 max-w-[1600px] mx-auto">
          {ALUMNI_DATA.map((alumni, index) => (
            <div 
              key={index} 
              className="alumni-card group relative aspect-[3/4] rounded-[2.5rem] overflow-hidden bg-white border border-slate-200 shadow-xl transition-all duration-500 hover:border-slate-300"
            >
              {/* Image Layer */}
              ...
              <div className="absolute inset-0 p-8 flex flex-col justify-end">
                <div className="mb-6 transform translate-y-8 group-hover:translate-y-0 transition-transform duration-500">
                  <span className="text-xs font-bold uppercase tracking-widest text-[#00c8c8] mb-2 block">
                    {alumni.batch}
                  </span>
                ...
                <div className="h-px w-full bg-white/20 mb-6 scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" />

                <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
                  <div className="flex items-start gap-3 mb-6">
                    <Quote size={20} className="text-[#00c8c8] shrink-0" />
                  ...
                  <div className="flex items-center gap-4">
                    <button className="p-2 rounded-full bg-white/10 border border-white/20 hover:bg-[#0066cc] hover:border-[#0066cc] transition-all duration-300 text-white">
                      <Linkedin size={16} />
                    </button>
                    <button className="p-2 rounded-full bg-white/10 border border-white/20 hover:bg-[#00c8c8] hover:border-[#00c8c8] transition-all duration-300 text-white">
                      <Twitter size={16} />
                    </button>
                    <button className="ml-auto flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-white transition-colors">
              ...
              <div className="absolute top-0 right-0 w-24 h-24 overflow-hidden">
                <div className="absolute top-[-10px] right-[-40px] w-32 h-8 bg-[#0066cc] rotate-45 flex items-center justify-center shadow-lg">
      ...
      {/* Stats Section */}
      <section className="py-40 px-6 border-t border-slate-200 bg-white">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-20 text-center">
          {[
            { label: "Global Alumni", value: "500+", color: "text-[#0066cc]" },
            { label: "Countries", value: "25+", color: "text-[#00c8c8]" },
      ...
      <section className="py-32 px-6 text-center relative overflow-hidden">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full bg-blue-50 blur-[150px] pointer-events-none" />
        <h2 className="text-4xl md:text-6xl font-black uppercase tracking-tighter mb-8 italic text-slate-900">
          JOIN THE <span className="text-[#0066cc]">LEGACY</span>
        </h2>
        ...
        <button className="px-12 py-4 bg-slate-900 text-white font-black uppercase tracking-widest rounded-full hover:bg-[#0066cc] transition-all duration-500 transform hover:scale-110">
          Reconnect Now
        </button>
      </section>
